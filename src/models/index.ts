...
const config: DbConfig = require(path.resolve(`${__dirname}./../config/config.json`));
let db = null;
...
if (!db) {
  ...
  const sequelize: Sequelize.Sequelize = new Sequelize(config[env].database, config[env].username, config[env].password, {
    ...config[env],
    storage,
    operatorsAliases
  });
